...
if [[ $__jade ]]; then
    echo "  build jade"
    jade --out ./qianxi/ ./qianxi-src/template/qianxi/*.jade
fi
...
if [[ $__stylus ]]; then
    echo "  build stylus"
    stylus --compress --out ./static/css/ ./qianxi-src/style/qianxi-upgrade.styl
fi
...
if [[ $__script ]]; then
    echo "  build script"
    component build --copy -o ./static/js/ --name app

    # Compress script
    if [[ $__script_compress ]]; then
        mv ./static/js/app.js ./static/js/app.js.source
        uglifyjs ./static/js/app.js.source --output ./static/js/app.js --compress --mangle --source-map ./static/js/app.js.map --source-map-url app.js.map  --source-map-include-sources
